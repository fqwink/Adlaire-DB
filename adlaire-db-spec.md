...
- **1 フェーズ = 1 開発単位**：実装・テスト・レビュー・マージはフェーズ単位で完結させる
- **前フェーズ完了が着手条件**：前フェーズの全テストケースが PASS してから次フェーズに着手する
- **1 フェーズ = 1 PR**：フェーズをまたいだ実装変更は原則として行わない
- 各フェーズの内製化計画はフェーズ完了後に検討する（§3.5.3）
