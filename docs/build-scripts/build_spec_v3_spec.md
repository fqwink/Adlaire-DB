...
`build_spec_v3.py` は、Adlaire DB プロジェクト憲章 / 仕様正本入口の Markdown ソースを単一の自己完結型 HTML ドキュメントへ変換する Python スクリプトである。外部ライブラリに依存せず、標準ライブラリ（`re`、`html`、`unicodedata`、`pathlib`、`typing`）のみで動作する。

### 1.1 目的

- プロジェクト憲章 / 仕様正本入口と Phase 分割ファイルを、快適に閲覧できる単一 HTML ドキュメントサイトへ変換する
...
| 入力 | UTF-8 エンコードの Markdown ファイル。`docs/PROJECT_CHARTER.md` から `docs/spec/core.md`、`docs/spec/auth.md`、`docs/spec/api.md`、`docs/spec/internalization.md`、`docs/spec/testing.md`、`docs/spec/phase-*.md` を include 展開する |
...
```python
<title>Adlaire DB プロジェクト憲章 / 仕様正本入口</title>          # ← ブラウザタブタイトル
<span class="hdr-title">Adlaire DB</span>  # ← ヘッダー表示名
<span class="hdr-ver">V.216</span>         # ← バージョンバッジ
```
...
Building TOC...
Assembling HTML...
Done -> docs/Adlaire-db-spec.html  (1,713,731 bytes / 1,673 KB)
```
