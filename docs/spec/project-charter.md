...
本仕様憲章は、Adlaire DB の実装・運用・互換性判断における最上位の仕様責務を固定する。`docs/spec/project-charter.md` は全体方針、共通契約、API、運用、実装統制、Phase include 順を定め、`docs/spec/phase-*.md` は Phase 別の実装契約を定める。仕様憲章と Phase 仕様が乖離する場合は、仕様矛盾として扱い、実装判断ではなく仕様改訂で解消する。

### 0.1 仕様バージョン管理責務

本仕様書のバージョンは `V.{累積番号}` 形式で表記する。現在の仕様書バージョンは `V.211` である。
...
本仕様書では、方針、ポリシー、仕様を以下の意味で区別する。これらは同じ文書内に共存できるが、実装時の拘束力と使い方を混同してはならない。

| 種別 | 意味 | 実装時の扱い |
|------|------|--------------|
| 方針 | 目指す方向、優先順位、進め方、将来の進化方向を示す記述 | 実装判断の前提にはなるが、方針だけでは実装完了条件にしない。実装する場合は仕様、契約、禁止事項、完了条件へ落とす |
| ポリシー | 条件に応じて許可、拒否、優先、抑止、保持、再試行、秘匿などを判定するルール | 仕様正本内に書かれたポリシーは、ポリシー型仕様として実装・テスト・レビュー対象にする |
| 仕様 | 実装者がそのまま実装、テスト、レビューできる確定契約 | method、path、request、response、error、永続化形式、状態遷移、権限判定、完了条件、禁止事項として最終判断に使う |

方針はポリシーではない。例えば「Turso Cloud 互換を優先する」「内製化は段階的に進める」は方針であり、それだけでは endpoint、error code、metadata schema、test artifact、Done 判定を確定しない。

ポリシーは仕様の外ではない。`write policy`、`retry policy`、`redaction policy`、`block policy`、`retention policy` のように、条件と判定結果が仕様正本に固定されている場合、それらはポリシー型仕様として扱う。ポリシー型仕様は、入力条件、判定順序、許可/拒否結果、エラー、永続化影響、証跡、テストを持たなければならない。

仕様は実装の最終契約である。方針やポリシーの表現だけで実装が迷う場合は、実装で補完してはならない。先に仕様本文へ API 契約、状態遷移、永続化契約、エラー契約、テスト条件、完了条件を追加してから実装する。
