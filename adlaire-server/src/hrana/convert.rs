...
use crate::{
    db::sqld_adapter::{SqlResult, SqlValue},
    error::AppError,
};
...
pub fn sql_to_stmt_result(r: SqlResult) -> StmtResult {
    let rows = r
        .rows
        .into_iter()
        .map(|row| row.into_iter().map(sql_val_to_hrana).collect())
        .collect();
    StmtResult {
        cols: r
            .cols
            .into_iter()
            .map(|(name, decltype)| Col { name, decltype })
            .collect(),
        rows,
...
fn sql_val_to_hrana(v: SqlValue) -> Value {
    match v {
        ...
        SqlValue::Integer(n) => Value::Integer {
            value: n.to_string(),
        },
