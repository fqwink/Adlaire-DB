...
pub async fn route(req: Request<Incoming>, state: SharedState) -> Result<HttpResponse, Infallible> {
...
pub async fn admin_route(
    ...
    let response = match (method.as_str(), path.as_str()) {
        ...
        ("POST", p) if is_turso_api_token_path(p) => {
            admin::platform::create_api_token(req, state).await
        }
        ("DELETE", p) if is_turso_api_token_path(p) => {
            admin::platform::revoke_api_token(req, state).await
        }
        ...
        ("PATCH", p) if is_turso_org_path(p) => {
            admin::platform::patch_organization(req, state).await
        }
        ("GET", p) if is_turso_usage_path(p) => {
            admin::platform::organization_usage(req, state).await
        }
        ...
        ("PATCH", p) if is_turso_group_configuration_path(p) => {
            admin::platform::patch_group_configuration(req, state).await
        }
        ("POST", p) if is_turso_group_rotate_path(p) => {
            admin::platform::rotate_group_tokens(req, state).await
        }
        ("POST", p) if is_turso_group_transfer_path(p) => {
            admin::platform::unsupported(req, state).await
        }
        ("GET" | "PATCH" | "DELETE", p) if is_turso_group_transfer_path(p) => {
        ...
        ("POST", p) if is_turso_databases_path(p) => {
            admin::platform::create_database(req, state).await
        }
        ("GET", p) if is_turso_database_path(p) => admin::platform::database(req, state).await,
        ("DELETE", p) if is_turso_database_path(p) => {
            admin::platform::delete_database(req, state).await
        }
        ("PATCH", p) if is_turso_database_configuration_path(p) => {
            admin::platform::patch_database_configuration(req, state).await
        }
        ("POST", p) if is_turso_database_token_path(p) => {
            admin::platform::create_database_token(req, state).await
        }
        ("POST", p) if is_turso_database_rotate_path(p) => {
            admin::platform::rotate_database_tokens(req, state).await
        }
        ("GET", p) if is_turso_database_stats_path(p) => {
            admin::platform::unsupported(req, state).await
        }
        ("POST" | "PATCH" | "DELETE", p) if is_turso_database_stats_path(p) => {
            admin::platform::unsupported_method(req, state).await
        }
        ("GET" | "POST" | "PATCH" | "DELETE", p) if is_turso_always_unsupported_path(p) => {
            admin::platform::unsupported(req, state).await
        }
        ("GET", p) if is_turso_unsupported_read_path(p) => {
            admin::platform::unsupported(req, state).await
        }
        ("POST" | "PATCH" | "DELETE", p) if is_turso_unsupported_write_path(p) => {
            admin::platform::unsupported_method(req, state).await
        }
        ("POST", "/v1/upload") => admin::platform::unsupported(req, state).await,
        ("GET" | "PATCH" | "DELETE", "/v1/upload") => {
            admin::platform::unsupported_method(req, state).await
        }
        ...
        ("POST", p) if p.ends_with("/restore/point-in-time") => {
            admin::backup::pitr(req, state).await
        }
        ...
        _ if path.starts_with("/v1/") => {
            Ok(crate::error::AppError::EndpointNotFound.into_response())
        }
        _ => Ok(not_found()),
    };
...
fn is_turso_group_configuration_path(path: &str) -> bool {
    matches!(
        segments(path).as_slice(),
        ["v1", "organizations", _, "groups", _, "configuration"]
    )
}
...
fn is_turso_group_rotate_path(path: &str) -> bool {
    matches!(
        segments(path).as_slice(),
        ["v1", "organizations", _, "groups", _, "auth", "rotate"]
    )
}
...
fn is_turso_group_transfer_path(path: &str) -> bool {
    matches!(
        segments(path).as_slice(),
        ["v1", "organizations", _, "groups", _, "transfer"]
    )
}
...
fn is_turso_database_path(path: &str) -> bool {
    matches!(
        segments(path).as_slice(),
        ["v1", "organizations", _, "databases", _]
    )
}
...
fn is_turso_database_configuration_path(path: &str) -> bool {
    matches!(
        segments(path).as_slice(),
        ["v1", "organizations", _, "databases", _, "configuration"]
    )
}
...
fn is_turso_database_token_path(path: &str) -> bool {
    matches!(
        segments(path).as_slice(),
        ["v1", "organizations", _, "databases", _, "auth", "tokens"]
    )
}
...
fn is_turso_database_rotate_path(path: &str) -> bool {
    matches!(
        segments(path).as_slice(),
        ["v1", "organizations", _, "databases", _, "auth", "rotate"]
    )
}
...
fn is_turso_database_stats_path(path: &str) -> bool {
    matches!(
        segments(path).as_slice(),
        ["v1", "organizations", _, "databases", _, "stats"]
    )
}
...
fn is_turso_unsupported_write_path(path: &str) -> bool {
    ...
    matches!(
        segs.as_slice(),
        ["v1", "organizations", _, "plans"] | ["v1", "organizations", _, "audit-logs"]
    )
}
